...
if (token) {
     ...
     * @type {Types.guild}
     */
    let g = guilds.find(g => g.id == serverid)
    //          Does the user have admin here?            Is the bot even in this server?s
    is_valid = ((parseInt(g.permissions) & 0x28) != 0) && bot_servers.some(b_g => b_g.id == g.id)
}
...
if (is_valid) {
    ...
    async function add_react_role() {
        let message = get_element(["#react-msg"]).value
        let emoji = get_element(["#react-emoji"]).value
        if (!message || !emoji || !isOnlyOneEmoji(emoji)) {
            console.log("wrong message or emoji")
            console.log(emoji.length)
            return
        }

        let role = get_element(["#react-role"]).value
        let channel = get_element(["#react-channel"]).value
        if (role == "-default" || channel == "-default") {
            console.log("wrong role or channel")
            return
        }

        let info = { message, emoji, role, channel }

        await notify_bot("reaction", info)
    }
    ...
    async function notify_bot(type, info) {
        ...
        let url = `${location.protocol}/api/notify-bot`
    ...
    async function main() {
        let base_url = `${location.protocol}/api/bot-info/${serverid}`
        let this_guild = bot_servers.find(g => g.id == serverid)
        let all_channels = await fetch(`${location.protocol}/api/channels/${this_guild.id}`).then(r => r.json())
        all_channels = all_channels.filter(c => c.type == 0) //0 for GUILD_TEXT
        let roles = await fetch(`${location.protocol}/api/roles/${this_guild.id}`).then(r => r.json())
        let audit_channel = await fetch(base_url + '/auditchannel').then(r => r.json())
        console.log(audit_channel)
        ...
        let react_channel = get_element(["#react-channel"])
        let react_role = get_element(["#react-role"])
        all_channels.forEach(channel => {
            let opt = document.createElement("option")
            opt.value = channel.id
            opt.textContent = channel.name
            react_channel.appendChild(opt)
        })
        roles.forEach(role => {
            let opt = document.createElement("option")
            opt.value = role.id
